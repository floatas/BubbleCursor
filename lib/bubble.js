(function () {
        ...
        extractQuery: (function (_this) {
            return function () {
                ...
                return function (searchUrl, url) {
                    ...
                    ref1 = searchUrl.split("%s"), searchUrl = ref1[0], suffixTerms = 2 <= ref1.length ? Array.prototype.slice.call(ref1, 1) : [];
            ...
            if (!isClickable && element.hasAttribute("jsaction")) {
                ...
                for (i = 0, len = jsactionRules.length; i < len; i++) {
                    ...
                    if ((1 <= (ref7 = ruleSplit.length) && ref7 <= 2)) {
                        ref8 = ruleSplit.length === 1 ? ["click"].concat(Array.prototype.slice.call(ruleSplit[0].trim().split(".")), ["_"]) : [ruleSplit[0]].concat(Array.prototype.slice.call(ruleSplit[1].trim().split(".")), ["_"]), eventType = ref8[0], namespace = ref8[1], actionName = ref8[2];
    ...
    var toggleBubble = function(code,ekey){
        if (code == ekey) {
            showCirc = !showCirc;

            chrome.storage.local.set({activeBubble: showCirc});
    ...
    document.addEventListener('keypress', function (e) {
        ...
        chrome.storage.sync.get(['showCircle'], function (obj) {
            return toggleBubble(code, obj.showCircle.keyCode);
        });

    }, false);
    
    chrome.storage.local.get(['activeBubble'], function(data){
